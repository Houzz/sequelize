...
class Query extends AbstractQuery {
  ...
  run(sql, parameters) {
    ...
    const instanceType = this.client.queryType === 'read' ? 'replica' : 'primary';
    ...
    if (benchmark) {
      ...
      this.sequelize.log(`Executing (${instanceTypeMessage}, ${this.client.uuid || 'default'}):`, this.sql, this.options);
    }
    ...
    return query.catch(err => {
      ...
      if (benchmark) {
        this.sequelize.log(`FAILED QUERY (${instanceTypeMessage}, ${this.client.uuid || 'default'}):`, this.sql, this.options);
      }
      ...
      .then(queryResult => {
        ...
        if (benchmark) {
          const elapsedTime = Date.now() - queryBegin;
          this.sequelize.log(`Executed ${elapsedTime > this.sequelize.options.queryThreshold ? 'SLOW QUERY ' : ''} (${instanceTypeMessage}, ${this.client.uuid || 'default'}):`, this.sql, elapsedTime, this.options);
        }
