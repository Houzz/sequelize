...
class Query extends AbstractQuery {
  ...
  run(sql, parameters) {
    ...
    return query.catch(err => {
      ...
      if (benchmark) {
        this.sequelize.log('FAILED QUERY (' + (this.client.uuid || 'default') + '): ' + this.sql, this.options);
      }
      ...
      .then(queryResult => {
        ...
        if (benchmark) {
          ...
          this.sequelize.log('Executed ' + (elapsedTime > this.sequelize.options.queryThreshold ? 'SLOW QUERY ' : '') + '(' + (this.client.uuid || 'default') + '): ' + this.sql, elapsedTime, this.options);
        }
